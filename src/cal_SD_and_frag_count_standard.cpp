...
tuple<vector<int>, vector<int>, vector<double>> processCellBedFile(
    const string& cellBed,
    ...
    vector<int> localCenterMatSum_vec;
    vector<int> localFlankMatSum_vec;
    ...
    for (size_t k = 0; k < cpool_vec.size(); k++) {
        ...
        assignCountPool_multiThread(localTagposmap, curCpool, colSum.data(), allHitArr, allHits, centerHitArr, centerHits, startIdx, endIdx, maxReadNum, colLen);
        
        // centerMatSum（center：±center_range）
        int centerMatSum = 0;
        for (int i = max(0, centerIdx - center_range); i < min(colLen, centerIdx + center_range + 1); ++i) {
            centerMatSum += colSum[i];
        }

        int flankMatSum = 0;
        for (int i = max(0, centerIdx - flank_range); i < min(colLen, centerIdx + flank_range + 1); ++i) {
            flankMatSum += colSum[i];
        }

        // int flankMatSum = 0;
        // for (int i = max(0, centerIdx - flank_range); i < max(0, centerIdx - center_range); ++i) {
        //     flankMatSum += colSum[i];
        // }

        // for (int i = min(colLen, centerIdx + center_range + 1); i < min(colLen, centerIdx + flank_range + 1); ++i) {
        //     flankMatSum += colSum[i];
        // }

        localCenterMatSum_vec.push_back(centerMatSum);
        localFlankMatSum_vec.push_back(flankMatSum);
    ...
    return make_tuple(localCenterMatSum_vec, localFlankMatSum_vec, localSummitDist_vec);
}
...
int main(int argc, char* const argv[]) {
    ...
    if (metaFile.empty() || regionPath.empty()){
        ...
        cerr << "-r:        center range (upstream and downstream) used in fragment count calculation, default: 100" << endl;
        cerr << "-k:        flank range (upstream and downstream) used in fragment count calculation, default: 200" << endl;
    ...
    vector<vector<int>> center_countSum_mat(totalCellNum);
    vector<vector<int>> flank_countSum_mat(totalCellNum);
    ...
    if (threadNum == 1) {
        ...
        for (size_t i = 0; i < totalCellNum; i++) {
            auto [centerMatSum_vec, flankMatSum_vec, summitDist_vec] = processCellBedFile(cellBedFiles[i], cpool_vec, 
                                                                                        colLen, ups, downs, win, motifShift,
                                                                                        covThre, sdReionReadHitThre, maxReadNum, 
                                                                                        myCenterRange, myFlankRange);
            summitDist_mat[i] = summitDist_vec;
            center_countSum_mat[i] = centerMatSum_vec;
            flank_countSum_mat[i] = flankMatSum_vec;
    ...
    // Output results for center and flank counts
    ...
    ofstream centerOut(centerMatSumFile);
    for (size_t i = 0; i < validCenterNum; i++){
        centerOut << centerName_vec[i] << "\t";
    }
    centerOut << "\n";

    ofstream flankOut(flankMatSumFile);
    for (size_t i = 0; i < validCenterNum; i++){
        flankOut << centerName_vec[i] << "\t";
    }
    flankOut << "\n";

    ofstream distOut(distfile);
    for (size_t i = 0; i < validCenterNum; i++){
        distOut << centerName_vec[i] << "\t";
    }
    distOut << "\n";
    ...
    for (size_t i = 0; i < totalCellNum; ++i) {
        ...
        centerOut << tmp_basename << "\t";
        for (size_t j = 0; j < center_countSum_mat[i].size(); ++j) {
            centerOut << center_countSum_mat[i][j] << "\t";
        }
        centerOut << "\n";

        flankOut << tmp_basename << "\t";
        for (size_t j = 0; j < flank_countSum_mat[i].size(); ++j) {
            flankOut << flank_countSum_mat[i][j] << "\t";
        }
        flankOut << "\n";

        distOut << tmp_basename << "\t";
        for (size_t j = 0; j < summitDist_mat[i].size(); ++j) {
            distOut << summitDist_mat[i][j] << "\t";
        }
        distOut << "\n";
    }
